...
try{
    if(fs.existsSync('canada.txt')){
        ...
        console.log('Deleted Sucecsfully')
    }
}
catch(err){
...
try{
    if(fs.existsSync('ind.txt')){
        fs.unlinkSync('ind.txt');
        console.log('Deleted Sucecsfully')
    }
}
catch(err){
    console.log('India doesnt exist');
}
...
.on('data',(data) => {
    ...
    if(data.country== 'India'){
        fs.appendFileSync('ind.txt',`${data.country},${data.year},${data.population}\n`)
    }
